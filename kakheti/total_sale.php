...
<body>
    ...
    <table width="100%">
        <tr>
            <td><a href="/kakheti/orders.php">შეკვეთები</a></td>
            <td><a href="/kakheti/all_info.php">ობიექტები</a></td>
            <td><p>რეალიზაცია თვეების მიხედვით</p></td>
            <td><p class="righttext"> (კახეთი) მომხმარებელი: <?php echo $logedUser['name']?> <a href="login.php?action=logout">გასვლა</a></p></td>
        </tr>
    </table>
    <div class="righttext" ><a href="https://www.apeni.ge/login.php">თბილისის გვერდზე გადასვლა</a>  </div>
...
FROM
    `moneyoutput`
WHERE YEAR(tarigi) > 2018 AND obieqtis_id NOT IN(185, 187, 192)  
GROUP BY
    YEAR(tarigi),
    MONTH(tarigi)
" ;
...
ON
    b.ludis_id = l.id
WHERE
	YEAR(b.tarigi) > 2018 AND obieqtis_id NOT IN(185, 187, 192)
GROUP BY
	YEAR(b.tarigi), 
    MONTH(b.tarigi),    
    dasaxeleba 
...
<script >
    ...
    function getDataforChart(yeartoshow){
        var url = location.protocol + "//apeni.ge/kakheti/monthly_results.php";
        if (yeartoshow != 0){
        ...
        success: function (response) {   
            ...
            // ******************** tanxebi **************************************
            $.ajax({
                url: location.protocol + "//apeni.ge/kakheti/monthly_results_M.php?year="+yy,
                method: 'get',
                dataType: 'json',
                success: function (response) {
                    var pr1 = [];
