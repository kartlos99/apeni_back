...
$sql_money = "
SELECT obieqtis_id, sum(tanxa) AS money_sum FROM `moneyoutput` WHERE date(`tarigi`) = '$dges'
GROUP BY obieqtis_id";

$rs = mysqli_query($con, $sql_money);
$moneyDaySum = [];
while ($row = mysqli_fetch_assoc($rs)) {
    $moneyDaySum[$row['obieqtis_id']] = $row['money_sum'];
}
...
while ($rs = mysqli_fetch_assoc($result)) {
    if (isset($moneyDaySum[$rs['obieqtis_id']])) {
        $rs['money'] = $moneyDaySum[$rs['obieqtis_id']];
        $moneyDaySum[$rs['obieqtis_id']] = 0;
    } else
        $rs['money'] = 0;
